 ...
 */
if ( ! function_exists( 'fawzi_vt_get_current_theme_update' ) ) {
  function fawzi_vt_get_current_theme_update( $updates ) {

    if ( isset( $updates->checked ) ) {

      if ( ! class_exists( 'Envato_Protected_API' ) ) {
        require_once( FAWZI_FRAMEWORK .'/plugins/theme-updater/class-envato-protected-api.php' );
      }
  ...
  add_filter( 'pre_set_site_transient_update_themes', 'fawzi_vt_get_current_theme_update' );
}

if ( ! function_exists( 'fawzi_vt_admin_head' ) ) {
  function fawzi_vt_admin_head() {
  ...
  add_action( 'admin_head', 'fawzi_vt_admin_head' );
}