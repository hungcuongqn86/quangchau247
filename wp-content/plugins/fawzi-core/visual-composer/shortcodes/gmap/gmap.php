...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('fwzi_gmap_function')) {
  function fwzi_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = FAWZI_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.fwzi-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. fawzi_core_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' fwzi-gmap-'. $e_uniqid;

    wp_enqueue_script( 'vtheme-googlemap', FAWZI_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', true );
    ...
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="fwzi-gmap-tab tab-pane fade">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="fwzi-google-map '. $custom_css .' '. $class .' '. $styled_class .'"></div>';
    echo $gmap_close;
...
add_shortcode( 'fwzi_gmap', 'fwzi_gmap_function' );
